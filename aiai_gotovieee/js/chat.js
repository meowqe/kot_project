// chat.js — ИИ-ассистент Serendale v4.4 (Умный плавный скролл)
...
class ChatUI {
    constructor() {
        ...
        this.isUserNearBottom = true; // Пользователь внизу?
        this.newMessageIndicator = null; // Индикатор новых сообщений
    ...
    initEventListeners() {
        ...
        if (this.chatMessages) {
            ...
            this.chatMessages.addEventListener('scroll', () => {
                const threshold = 80; // Увеличил порог для более точного определения
                ...
                // Скрываем индикатор, если пользователь докрутил до низа
                if (this.isUserNearBottom && this.newMessageIndicator) {
    ...
    addMessage(textOrElement, isUser = false) {
        ...
        if (this.isUserNearBottom || isUser) {
            this.scrollToBottom();
        } else {
    ...
    showNewMessageIndicator() {
        ...
        this.newMessageIndicator.innerHTML = `
            <button class="scroll-to-bottom-btn">
                <i class="fa-solid fa-arrow-down"></i> Новые сообщения
            </button>
        `;

        this.newMessageIndicator.querySelector('.scroll-to-bottom-btn').addEventListener('click', () => {
            this.scrollToBottom();
        });

        this.chatMessages.appendChild(this.newMessageIndicator);
    }
    ...
    injectStyles() {
        if (document.getElementById('sr-styles')) {
            const styleEl = document.getElementById('sr-styles');
            styleEl.textContent = this._getDynamicStyles();
            return;
        }
        const style = document.createElement('style');
        style.id = 'sr-styles';
        style.textContent = this._getDynamicStyles();
    ...
    _getDynamicStyles() {
        return `
            ...
            .scroll-to-bottom-btn:hover {
                ...
                border-color: rgba(252,79,246,.5);
            }
            
            /* Единый стиль для карточек тарифов */
            .sr-card {
            ...
            .sr-card .price {
                font-size: 24px;
                font-weight: 700;
                background: linear-gradient(135deg, #5c6df3, #ff3bff);
            ...
            /* Контейнер сообщения для карточек */
            .message-content:has(.sr-card) {
                width: 100% !important;
            ...
            .sr-btn:hover {
                ...
                box-shadow: 0 8px 20px rgba(252,79,246,.3);
            }
            ...
            .sr-info-block p, .sr-info-block li {
                font-size: 13px;
...
// ... (ВЕСЬ ОСТАЛЬНОЙ КОД ОСТАЁТСЯ БЕЗ ИЗМЕНЕНИЙ, начиная с const knowledgeBase = {...})

// =============================================
// 2. БАЗА ЗНАНИЙ (без изменений)
// =============================================
const knowledgeBase = {
    // ... (вся база знаний остаётся без изменений)
    tariffs: [
        { id: 'lite', name: 'Лайт', price: 399, gb: 15, minutes: 400, sms: 100, features: ['Безлимит на соцсети', 'eSIM бесплатно', 'Защита от спама'], bestFor: ['Социальные сети', 'Мессенджеры', 'Экономия'], icon: 'fa-feather', color: '#60a5fa' },
        { id: 'optima', name: 'Оптима', price: 599, gb: 30, minutes: 800, sms: 300, features: ['Безлимит на YouTube / VK / Музыку', 'Кешбэк 5%', 'Облачное хранилище 10 ГБ'], bestFor: ['Видео', 'Музыка', 'Работа', 'Учёба'], icon: 'fa-bolt', color: '#fbbf24' },
        { id: 'max', name: 'Максимум', price: 899, gb: Infinity, minutes: Infinity, sms: Infinity, features: ['Безлимит на всё', '5G везде', 'Приоритетная поддержка 24/7', 'Спутниковая связь'], bestFor: ['Стриминг', 'Игры', 'Максимальная скорость', 'Путешествия по РФ'], icon: 'fa-rocket', color: '#f472b6' },
        { id: 'family', name: 'Семейный', price: 1299, gb: 100, minutes: 2000, sms: 500, features: ['До 4 сим-карт', 'Общий интернет', 'Родительский контроль', 'Семейный чат'], bestFor: ['Семья', 'Дети', 'Общий бюджет'], icon: 'fa-people-group', color: '#a78bfa' },
        { id: 'business', name: 'Бизнес', price: 1999, gb: 'Безлимит', minutes: 'Безлимит', sms: 'Безлимит', features: ['До 10 сим-карт', 'Выделенный менеджер', 'Корпоративная сеть', 'Приоритетная поддержка'], bestFor: ['Бизнес', 'Команда', 'Корпоративная связь'], icon: 'fa-briefcase', color: '#34d399' }
    ],
    esimInfo: [
    ...
    tariffRecommendations: {
        'соцсети': { tariff: 'Лайт', reason: 'Идеален для Instagram, VK, Telegram с безлимитным трафиком на соцсети' },
        'мессенджеры': { tariff: 'Лайт', reason: 'Безлимит на WhatsApp, Telegram — всё, что нужно для общения' },
        'видео': { tariff: 'Оптима', reason: 'Безлимит на YouTube и VK Видео на скорости до 300 Мбит/с' },
        'музыка': { tariff: 'Оптима', reason: 'Потоковая музыка без ограничений + кешбэк 5% на подписки' },
        'стриминг': { tariff: 'Максимум', reason: 'Безлимит на всё + 5G для стримов в 4K' },
        'игры': { tariff: 'Максимум', reason: 'Минимальный пинг и максимальная скорость — идеал для гейминга' },
        'семья': { tariff: 'Семейный', reason: 'До 4 номеров, общий интернет и родительский контроль' },
        'дети': { tariff: 'Семейный', reason: 'Родительский контроль и общий бюджет для всей семьи' },
        'бизнес': { tariff: 'Бизнес', reason: 'До 10 номеров, выделенный менеджер и корпоративная сеть' },
        'работа': { tariff: 'Оптима', reason: 'Достаточно интернета для видеоконференций и удалённой работы' },
        'учёба': { tariff: 'Оптима', reason: 'Облачное хранилище и безлимит на образовательные платформы' },
        'путешествия по россии': { tariff: 'Максимум', reason: 'Покрытие 99% РФ + спутниковая связь в удалённых районах' },
        'путешествия за границу': { tariff: 'Максимум', reason: 'Доступен роуминг в 190+ странах с 5G' },
        'экономия': { tariff: 'Лайт', reason: 'Всего 399 ₽ — лучшее соотношение цена/качество' },
        'премиум': { tariff: 'Максимум', reason: 'Всё включено: безлимиты, 5G, приоритетная поддержка' },
        'команда': { tariff: 'Бизнес', reason: 'Корпоративная связь до 10 сотрудников с общим управлением' }
    }
};
...
function getTariffByName(name) {
    ...
    const aliases = {
        'лайт': 'lite', 'light': 'lite',
        'оптима': 'optima', 'оптимум': 'optima', 'optimal': 'optima',
        'макс': 'max', 'максимум': 'max', 'максимальный': 'max', 'maximum': 'max',
        'семейный': 'family', 'семья': 'family', 'family': 'family',
        'бизнес': 'business', 'корпоративный': 'business', 'business': 'business'
    };
...
function generateTariffCard(tariff, mode = 'consult') {
    ...
    const bestFor = tariff.bestFor.map(f => `<span style="display:inline-block;background:rgba(255,255,255,.08);padding:4px 10px;border-radius:12px;font-size:11px;margin:2px;">${f}</span>`).join(' ');

    let buttonHTML = '';
    if (mode === 'consult') {
        buttonHTML = `<button class="sr-btn consult" data-action="send" data-value="Расскажи подробнее про тариф ${tariff.name}">📊 Подробнее о тарифе</button>`;
    } else if (mode === 'order') {
        buttonHTML = `<button class="sr-btn order" data-action="send" data-value="Оформить тариф ${tariff.name}">✅ Оформить этот тариф</button>`;
    }

    return `<div class="sr-card" style="border-left: 3px solid ${tariff.color}; width: 100%; box-sizing: border-box;">
        <h4 style="margin:0">
            <i class="fa-solid ${tariff.icon}" style="font-size:20px;background:linear-gradient(135deg,${tariff.color},${tariff.color}88);-webkit-background-clip:text;-webkit-text-fill-color:transparent;"></i> 
            ${tariff.name}
        </h4>
        <div class="price">${tariff.price} ₽ / мес</div>
        <div style="margin:8px 0;font-size:12px;color:rgba(255,255,255,.5);">${bestFor}</div>
        <ul>
            <li><i class="fa-solid fa-wifi"></i> Интернет: ${gb}</li>
            <li><i class="fa-solid fa-phone"></i> Звонки: ${min}</li>
            <li><i class="fa-solid fa-comment"></i> SMS: ${sms}</li>
            ${feats}
        </ul>
        ${buttonHTML}
    </div>`;
...
// =============================================
// 3. NLU (без изменений в основной логике)
// =============================================
class NLU {
    getIntent(msg) {
        ...
        // ===== ОФОРМИТЬ ТАРИФ (проверяем до сравнения) =====
        const orderMatch = msg.match(/оформить тариф (.+)/i) || msg.match(/хочу оформить (.+)/i) || msg.match(/оформи (.+)/i);
        if (orderMatch) {
            const tariffName = orderMatch[1].toLowerCase().trim();
            const tariff = getTariffByName(tariffName);
            if (tariff) {
                return { intent: 'order_tariff', tariff: tariff.name };
            }
        }

        // ===== ХОЧУ ОФОРМИТЬ ЭТОТ ТАРИФ =====
        if (this._match(msg, ['хочу оформить этот тариф', 'хочу оформить тариф', 'оформить этот тариф', 'оформить тариф', 'хочу подключить этот тариф', 'подключить этот тариф', 'хочу подключить тариф', 'подключить тариф'])) {
            return { intent: 'order_current_tariff' };
        }

        // ===== СРАВНЕНИЕ ТАРИФОВ =====
        if (this._match(msg, ['сравнить тарифы', 'сравнение тарифов', 'сравни тарифы', 'сравнить', 'сравнение', 'что лучше', 'какой тариф лучше', 'какой выбрать', 'помоги выбрать', 'подбери тариф', 'посоветуй тариф', 'рекомендуй тариф', 'какой тариф подойдёт', 'какой тариф подойдет', 'не знаю какой выбрать', 'подобрать под мои нужды', 'что посоветуешь', 'посоветовать тариф'])) {
            return { intent: 'compare_tariffs' };
        }

        // ===== ПОДРОБНЕЕ О ТАРИФЕ =====
        const detailMatch = msg.match(/расскажи подробнее про тариф (.+)/i) || msg.match(/подробнее о тарифе (.+)/i) || msg.match(/подробнее про (.+)/i);
        if (detailMatch) {
            const tariffName = detailMatch[1].toLowerCase().trim();
            const tariff = getTariffByName(tariffName);
            if (tariff) {
                return { intent: 'tariff_detail', tariff: tariff.name };
            }
        }
        ...
        const needsMatch = this._detectNeeds(msg);
        if (needsMatch) {
            return { intent: 'needs_analysis', needs: needsMatch };
        }
        ...
        if (this._has(msg, ['сравни', 'сравнение', 'посоветуй', 'рекомендуй', 'подбери', 'какой лучше', 'не знаю'])) return { intent: 'compare_tariffs' };
        if (this._has(msg, ['оформить этот тариф', 'оформить тариф', 'хочу оформить', 'подключить этот'])) return { intent: 'order_current_tariff' };
    ...
    _detectNeeds(msg) {
        const needsMap = {
            'соцсети': ['соцсет', 'соц сет', 'социальн', 'инстаграм', 'instagram', 'вконтакте', 'vk', 'tiktok', 'тикток'],
            'мессенджеры': ['мессенджер', 'общени', 'чат', 'whatsapp', 'телеграм', 'telegram', 'вайбер', 'viber'],
            'видео': ['видео', 'youtube', 'ютуб', 'смотреть', 'кино', 'сериал', 'фильм', 'netflix', 'нетфликс'],
            'музыка': ['музык', 'музыка', 'яндекс музык', 'spotify', 'спотифай', 'аудио', 'подкаст'],
            'стриминг': ['стрим', 'стриминг', 'трансляц', 'веду эфир', 'twitch', 'твич'],
            'игры': ['игр', 'игра', 'гейм', 'пс5', 'ps5', 'xbox', 'steam', 'стим', 'киберспорт'],
            'семья': ['семь', 'семей', 'родственик', 'родные', 'муж', 'жена'],
            'дети': ['ребен', 'дети', 'ребёнок', 'сын', 'дочь', 'подросток'],
            'бизнес': ['бизнес', 'компани', 'фирм', 'корпорат', 'офис'],
            'работа': ['работ', 'удалён', 'удален', 'офис', 'сотрудник', 'фриланс'],
            'учёба': ['учёб', 'учеб', 'универ', 'студент', 'школ', 'образован'],
            'путешествия по россии': ['путешеств по росси', 'поезд по росси', 'крым', 'сочи', 'алтай', 'байкал', 'дальний восток'],
            'путешествия за границу': ['за границ', 'зарубеж', 'путешеств', 'тур поезд', 'авиаперелёт', 'авиаперелет'],
            'экономия': ['эконом', 'дешев', 'бюджет', 'недорог', 'дешёв'],
            'премиум': ['премиум', 'всё включен', 'все включен', 'максимум', 'полный'],
            'команда': ['команд', 'коллектив', 'сотрудник', 'работник', 'отдел']
        };

        for (const [need, keywords] of Object.entries(needsMap)) {
...
class DialogManager {
    ...
    handleUserMessage(text) {
        ...
        // ===== ИСПРАВЛЕНИЕ: ROAMING_WAIT — проверяем, не команда ли это =====
        if (this.state === 'ROAMING_WAIT') {
            const country = this.nlu._country(text) || text.trim();

            // Проверяем, не является ли ввод известной командой (баланс, тарифы и т.д.)
            const knownCommands = [
                'баланс', 'тарифы', 'esim', 'роуминг', 'перенос', 'документы',
                'покрытие', 'услуги', 'контакты', 'приложение', 'главное меню', 'меню',
                'помощь', 'справка', 'назад', 'отмена', 'привет', 'пока'
            ];

            const isCommand = knownCommands.some(cmd => text.toLowerCase().includes(cmd));

            if (isCommand) {
                // Выходим из режима ожидания и обрабатываем как обычную команду
                this.state = 'IDLE';
            ...
            // Если это не команда, обрабатываем как страну
            this._respond(() => {
                this._roamingCountry(country);
                this.state = 'IDLE';
    ...
    _handle(text) {
        const result = this.nlu.getIntent(text);
        const { intent, tariff, country, needs } = result;
        if (tariff) {
            this.data.tariff = tariff;
        ...
        this._respond(() => {
            switch (intent) {
                ...
                case 'show_tariffs': this._showAllTariffs(); break;
                case 'compare_tariffs': this._compareTariffs(); break;
                case 'needs_analysis': this._analyzeNeeds(needs); break;
                case 'tariff_detail': this._tariffDetail(tariff); break;
                case 'show_single_tariff': this._showSingleTariff(tariff); break;
                case 'order_tariff': this._startOrderTariff(tariff); break;
                case 'order_current_tariff': this._orderCurrentTariff(); break;
    ...
    _handleYes() {
        switch (this.context) {
            case 'tariffs': this._compareTariffs(); break;
    ...
    _fallback() {
        switch (this.context) {
            case 'tariffs':
                this._say('Расскажите, для каких целей вам нужна связь — и я помогу подобрать лучший вариант!', ['Соцсети', 'Видео и музыка', 'Игры', 'Семья', 'Бизнес', 'Все тарифы']);
                break;
            case 'esim':
            ...
            case 'contacts':
                ...
                break;
            default:
                this._say('🤔 Расскажите, для каких целей вам нужна связь?\nЯ помогу подобрать идеальный тариф! ✨', ['Соцсети и мессенджеры', 'Видео и стриминг', 'Игры', 'Семья', 'Бизнес', 'Главное меню']);
                break;
        }
    }

    // ===== ПОКАЗ ВСЕХ ТАРИФОВ =====
    // В классе DialogManager, метод _showAllTariffs:
    _showAllTariffs() {
        this.context = 'tariffs';
        this.selectedTariff = null;

        // Больше не отключаем автоскролл — он сам умный
        // this.ui.disableAutoScroll(); ❌ Убираем!

        this.ui.addMessage('📱 <b>Наши тарифы:</b>\n\nВыберите интересующий тариф или расскажите о своих потребностях — и я помогу с выбором! 😊', false);

        knowledgeBase.tariffs.forEach(t => {
            const c = document.createElement('div');
            c.innerHTML = generateTariffCard(t, 'consult');
            this.ui.addMessage(c.firstElementChild, false);
        });

        // Убираем принудительное включение, оно больше не нужно
        // setTimeout(() => { this.ui.enableAutoScroll(); }, 300); ❌ Убираем!
    ...
    _compareTariffs() {
        this.context = 'tariffs';
        this.ui.addMessage('📊 <b>Давайте подберём идеальный тариф!</b>\n\nРасскажите, для каких целей вам нужна связь?', false);

        const needsGrid = document.createElement('div');
        needsGrid.innerHTML = `
            <div class="sr-grid">
                <div class="sr-grid-item" data-action="send" data-value="Нужны соцсети и мессенджеры">
                    <i class="fa-solid fa-heart"></i>
                    <p>Соцсети и мессенджеры</p>
                </div>
                <div class="sr-grid-item" data-action="send" data-value="Смотрю видео и слушаю музыку">
                    <i class="fa-solid fa-play"></i>
                    <p>Видео и музыка</p>
                </div>
                <div class="sr-grid-item" data-action="send" data-value="Играю в онлайн-игры">
                    <i class="fa-solid fa-gamepad"></i>
                    <p>Игры и стриминг</p>
                </div>
                <div class="sr-grid-item" data-action="send" data-value="Нужен тариф для семьи">
                    <i class="fa-solid fa-people-group"></i>
                    <p>Семья</p>
                </div>
                <div class="sr-grid-item" data-action="send" data-value="Для бизнеса и команды">
                    <i class="fa-solid fa-briefcase"></i>
                    <p>Бизнес и команда</p>
                </div>
                <div class="sr-grid-item" data-action="send" data-value="Путешествую и нужен роуминг">
                    <i class="fa-solid fa-earth-americas"></i>
                    <p>Путешествия</p>
                </div>
            </div>
            <p style="font-size:13px;color:rgba(255,255,255,.6);margin-top:10px;">Или напишите своими словами — я пойму! 😊</p>
        `;
        this.ui.addMessage(needsGrid, false);
        this.ui.setSuggestions([
            'Соцсети', 'Видео', 'Игры', 'Семья', 'Бизнес', 'Путешествия', 'Все тарифы', 'Главное меню'
        ]);
    }

    _analyzeNeeds(need) {
        this.context = 'tariffs';
        const recommendation = knowledgeBase.tariffRecommendations[need];

        if (recommendation) {
            const tariff = knowledgeBase.tariffs.find(t => t.name === recommendation.tariff);
            this.selectedTariff = tariff.name;

            this.ui.addMessage(`🎯 <b>Отличный выбор!</b>\n\n${recommendation.reason}\n\nВот тариф, который идеально подходит:`, false);

            const c = document.createElement('div');
            c.innerHTML = generateTariffCard(tariff, 'order');
            this.ui.addMessage(c.firstElementChild, false);

            this.ui.setSuggestions([
                'Сравнить все тарифы',
                `Расскажи подробнее про тариф ${tariff.name}`,
                'Все тарифы',
                'Главное меню'
            ]);
        } else {
            this._compareTariffs();
        }
    }
    ...
    _tariffDetail(name) {
        ...
        if (tariff) {
            ...
            this.ui.addMessage(`📱 <b>Подробнее о тарифе «${tariff.name}»:</b>`, false);
            const c = document.createElement('div');
            c.innerHTML = generateTariffCard(tariff, 'order');
            this.ui.addMessage(c.firstElementChild, false);

            let detailText = `<div class="sr-info-block"><h4>🎯 Для кого подходит:</h4><p>${tariff.bestFor.join(', ')}</p>`;

            const otherTariffs = knowledgeBase.tariffs.filter(t => t.id !== tariff.id);
            const cheaper = otherTariffs.find(t => t.price < tariff.price);
            const moreExpensive = otherTariffs.find(t => t.price > tariff.price);

            if (cheaper) {
                detailText += `<h4 style="margin-top:10px;">💰 Экономия:</h4><p>Если хотите сэкономить, посмотрите тариф «${cheaper.name}» за ${cheaper.price} ₽/мес.</p>`;
            }
            if (moreExpensive) {
                detailText += `<h4 style="margin-top:10px;">🚀 Больше возможностей:</h4><p>Для максимальных возможностей — «${moreExpensive.name}» за ${moreExpensive.price} ₽/мес.</p>`;
            }

            detailText += '</div>';
            this.ui.addMessage(detailText, false);
            this.ui.setSuggestions([
                'Хочу оформить этот тариф',
                'Сравнить все тарифы',
                'Все тарифы',
    ...
    _startOrderTariff(tariffName) {
        const tariff = getTariffByName(tariffName);
        if (tariff) {
            this.selectedTariff = tariff.name;
            this.data.tariff = tariff.name;
            this.data.type = 'tariff';
            this._say(`✅ <b>Оформление тарифа «${tariff.name}»</b>\n\nСейчас заполним несколько полей — и заявка уйдёт менеджеру.`);
            this.state = 'ORDERING';
            this.context = 'tariff';
            this.data = { type: 'tariff', tariff: tariff.name, name: '', phone: '', email: '', addr: '', mnp: '', step: 0, total: 0 };
            this.data.total = 3;
    ...
    _orderCurrentTariff() {
        if (this.selectedTariff) {
            this._startOrderTariff(this.selectedTariff);
        } else {
            this._say('🤔 Какой тариф вас интересует?', ['Лайт', 'Оптима', 'Максимум', 'Семейный', 'Бизнес', 'Все тарифы']);
        }
    }
    ...
    _roaming() {
        ...
        this.ui.setInputPlaceholder('Название страны...');
        this.ui.setSuggestions(['Турция', 'ОАЭ', 'Германия', 'Таиланд', 'США', 'В каких странах доступен роуминг?', 'Главное меню']);
    ...
    _menu(msg) {
        ...
        this.ui.setSuggestions(['Тарифы', 'Помоги выбрать тариф', 'eSIM', 'Перенос номера', 'Роуминг', 'Документы', 'Баланс']);
    }
}

// =============================================
// 5. API-МОСТ (без изменений)
// =============================================
class API_Bridge {
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        console.log('✅ Serendale AI v4.2 готов (исправлен скролл + контекст роуминга)');
